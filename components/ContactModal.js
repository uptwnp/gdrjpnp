...
const ContactModal = ({ isOpen, onClose, type, projectName }) => {
  ...
  const getWhatsAppMessage = () => {
    const baseMessage = `Hi! I'm interested in ${projectName || 'your project'}.`;
  ...
  const getModalTitle = () => {
    switch (type) {
      ...
      case 'whatsapp': return 'Connect on WhatsApp';
      case 'pricing': return `Get Pricing - ${projectName}`;
      case 'callback': return 'Request a Callback';
      case 'video': return `Watch ${projectName} Video`;
  ...
  const getModalIcon = () => {
    switch (type) {
      case 'call': return <Phone className="h-6 w-6 text-blue-600" />;
      case 'whatsapp': return <MessageSquare className="h-6 w-6 text-green-600" />;
      case 'pricing': return <IndianRupee className="h-6 w-6 text-blue-600" />;
      case 'callback': return <Phone className="h-6 w-6 text-blue-600" />;
      case 'video': return <div className="h-6 w-6 bg-red-500 rounded-full flex items-center justify-center shadow-md"><span className="text-white text-xs font-bold ml-0.5">▶</span></div>;
      default: return <Phone className="h-6 w-6 text-blue-600" />;
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        <div className="fixed inset-0 transition-opacity bg-gray-500 bg-opacity-75" onClick={showSuccess ? resetForm : onClose}></div>
        ...
        <div className="inline-block w-full max-w-md p-6 my-8 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl">
          {showSuccess ? (
            <div className="text-center">
              ...
              <p className="text-gray-600 mb-6">
                Your details have been sent to our team and you will be contacted soon.
              </p>
              
              <div className="space-y-3 mb-6">
                <p className="text-sm text-gray-500">While you wait, you can also:</p>
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center">
              {getModalIcon()}
              <h3 className="ml-3 text-lg font-semibold text-gray-900">{getModalTitle()}</h3>
            </div>
            <button
              onClick={resetForm}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            >
              {type === 'whatsapp' ? 'Send WhatsApp Message' : 
               type === 'video' ? 'Watch Video Now' : 
               'Submit Request'}
            </button>
          </form>
